...
class Router
{
     ...
     */
    private function before($methods, $pattern, $fn)
    {
     ...
     * @param object|callable $fn      The handling function to be executed
     */
    public function get($pattern, $fn, array $actions = null)
    {
        /**
         * Checking if the current route has any specific actions
         * Example before => App\Middleware\Auth::class
         */
        if( $actions && is_array($actions)) {

            if( isset($actions['before']) ) {
                $this->before('GET', $pattern, $actions['before']);
            }
        }
    ...
    private function invoke($fn, $params = [])
    {
        ...
        /**
         * First, check if the route has a middleware attached by checking
         * the namespace if contains the 'Middleware' keyword
         * @see Illuminate\Support\Str
         */
        if( ! $fn instanceof \Closure ) {
            if( Str::contains($fn, 'Middleware')) {
                $this->runControllerMethod($fn, $params, false);
            }
        }

        if (is_callable($fn)) {
            // Handle simple closure        
            call_user_func_array($fn, $params);
        
        } elseif (stripos($fn, '@') !== false) {
            // If not, check the existence of special parameters
            $this->runControllerMethod($fn, $params);
        }
    }

    /**
     * Separate the callables
     */
    private function parseControllerAndMethod($fn)
    {
     ...
     * Running callables
     */
    private function runControllerMethod($fn, $params, $echo = true)
    {
            $callable = $this->parseControllerAndMethod($fn);

            $controller = $callable['controller'];
            $method = $callable['method'] !== null ? $callable['method'] : 'handle';
            ...
            if (class_exists($controller)) {
                ...
                if( is_callable([new $controller, $method]) ) {
                    ...
                    foreach($parameters as $param)
                    {
                        ...
                        if( $class = $param->getClass()->name ) {
                            $dependencies[$param->name] = new $class();
                        } else {
