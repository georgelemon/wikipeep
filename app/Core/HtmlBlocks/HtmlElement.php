...
namespace Dashboard\Support\HtmlBlocks;
...
class HtmlElement
{
    ...
    protected static function sanitizeTag($tag)
    {
    ...
    /**
     * Clean up given attributes
     * @return [type] [description]
     */
    protected static function sanitizeAttributes($attributes)
    {
        // return $attributes;
        if( is_string($attributes) ) {
            return strip_tags($attributes, ['ul', 'li', 'a', 'svg', 'circle', 'path']);
        }

        if( is_array($attributes) ) {
            $attr = [];
            foreach ($attributes as $key => $attribute) {
                $attr[] = strip_tags($attribute, ['ul', 'li', 'a', 'svg', 'circle', 'path']);
            }
